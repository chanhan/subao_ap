...
namespace Schedules
{
    public partial class FrmMain
    {
        ...
        private Dictionary<string, GameInfo> GetSchedulesByPCL(int allianceID, string gameType, bool acH = false)
        {
            ...
            DateTime startDate =this.dtpPCLSDate.Value.Date;
            DateTime endDate = this.dtpPCLEDate.Value.Date;
            ...
            while (startDate.CompareTo(endDate) <= 0)
            {
                ...
                if (DateTime.TryParse(gameDateStr, out gameDate))
                {
                    ...
                    // 下載資料
                    string xmlText = web.DownloadString("http://www.milb.com/lookup/xml/named.schedule_vw_complete.bam?game_date='" + gameDate.ToString("yyyy/MM/dd").Replace("-", "/") + "'&season=" + gameDate.ToString("yyyy") + "&league_id=112");
                    // 判斷網頁完成
